...
function initializeLaunchDarklyWithTimeout(sdkKey, initTimeout, additionalOptions) {
...
async function waitForInitializationWithTimeout(ldClient, initTimeout) {
    try {
        ...
        if (e instanceof DeadlineError) {
            console.log("[LaunchDarkly] Initialization timed out. Flags will return fallback values until initialization completes", e)
        } else {
...
function initializeWithMetrics(sdkKey, options={}) {
    ...
    performance.measure("launchdarkly:timeToInitStart", performance.nodeTiming.nodeStart)
    async function recordInitLatency(ldClient) { 
        try {
            await ldClient.waitForInitialization()
            const initComplete = performance.mark("launchdarkly:initComplete", perfOptions)
            performance.measure("launchdarkly:initLatency", "launchdarkly:initStarted")
            performance.measure("launchdarkly:timeToInitComplete", performance.nodeTiming.nodeStart)
            console.info(`[LaunchDarkly] Initialization successful! ${useLDD ? "The persistent store is available" : "Flags are now available." }`)
        } catch (e) {
            const initFailed = performance.mark("launchdarkly:initComplete", {detail: {success: false, error: e}})
            performance.measure("launchdarkly:initLatency", "launchdarkly:initStarted")
            performance.measure("launchdarkly:timeToInitComplete", performance.nodeTiming.nodeStart)
            console.error("[LaunchDarkly] Initialization failed permanently. Flags will return fallback values",e)
        }
    }
...
function getLDConfig() {
    ...
    function daemonModeConfig() {
        ...
        console.debug("[LaunchDarkkly] Using daemon mode. Flags/segments will be fetched on-demand during evaluation from ", protocol, " and cached for ", cacheTTL, " seconds")
        console.debug("[LaunchDarkkly] No streaming or polling connection will be established.")
    ...
    return Object.assign({}, loggingConfig(), endpointConfig(), daemonModeConfig(), eventConfig(), streamConfig())
}
...
function deadline(promise, timeoutDuration) {
    ...
    const timeout = new Promise((resolve, reject) => {
        timeoutHandle = setTimeout(() => reject(new DeadlineError()), timeoutDuration)
    })
